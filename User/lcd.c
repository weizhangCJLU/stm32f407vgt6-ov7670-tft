...
void LCD_SetCursor2(u16 Xpos1, u16 Ypos1,u16 Xpos2, u16 Ypos2)
{	 
 	if(lcddev.id==0X9341)
	{		    
		LCD_WR_REG(lcddev.setxcmd); 
		LCD_WR_DATA(Xpos1>>8);LCD_WR_DATA(Xpos1&0XFF);
    LCD_WR_DATA(Xpos2>>8);LCD_WR_DATA(Xpos2&0XFF);		
		LCD_WR_REG(lcddev.setycmd); 
		LCD_WR_DATA(Ypos1>>8);LCD_WR_DATA(Ypos1&0XFF); 
    LCD_WR_DATA(Ypos2>>8);LCD_WR_DATA(Ypos2&0XFF);		
	} 
} 
//设置LCD的自动扫描方向
...
{ 	
  ...
  GPIO_Init(GPIOE, &GPIO_InitStructure);//初始化  
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;//PD7,CS
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用输出
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
  ...
  GPIO_PinAFConfig(GPIOD,GPIO_PinSource0,GPIO_AF_FSMC);//PD0,AF12,FSMC_D2
  GPIO_PinAFConfig(GPIOD,GPIO_PinSource1,GPIO_AF_FSMC);//PD1,AF12,FSMC_D3
  ...
  GPIO_PinAFConfig(GPIOD,GPIO_PinSource15,GPIO_AF_FSMC);//PD15,AF12,FSMC_D1
 
  GPIO_PinAFConfig(GPIOE,GPIO_PinSource7,GPIO_AF_FSMC);//PE7,AF12,FSMC_D4
  ...
  GPIO_PinAFConfig(GPIOE,GPIO_PinSource15,GPIO_AF_FSMC);//PE15,AF12,FSMC_D12
 
  GPIO_PinAFConfig(GPIOD,GPIO_PinSource11,GPIO_AF_FSMC);//PD11,AF12,FSMC_A16->RS
  ...
  FSMC_NORSRAMCmd(FSMC_Bank1_NORSRAM1, ENABLE);  // 使能BANK1 
	...
	if(lcddev.id==0X9341)//如果是这几个IC,则设置WR时序为最快
	{
		//重新配置写时序控制寄存器的时序   	 							    
...
void LCD_Clear(u16 color)
{
	...
	LCD_SetCursor(0x00,0x0000);	//设置光标的起始位置 
	LCD_WriteRAM_Prepare();     		//开始写入GRAM	 	  
	for(index=0;index<totalpoint-38400;index++)
	{
		LCD->LCD_RAM=color;	
	}
}  
...
//size:字体大小
//*p:字符串起始地址		  
void LCD_ShowString(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p)//LCD_ShowString(30,130,200,12,12,"2015/11/8");	
{         
	u8 x0=x;
	width+=x;
	height+=y;
...
void LCD_DisplayTest(u16 color1,u16 color2)
{
	...
	LCD_SetCursor(0x00,0x0000);	//设置光标的起始位置 
	LCD_WriteRAM_Prepare();     		//开始写入GRAM	 	  
	for(index=0;index<38400;index++)
	{
		LCD->LCD_RAM=color1;	
	}
	for(index=38400;index<76800;index++)
	{
		LCD->LCD_RAM=color2;	
	}
}
